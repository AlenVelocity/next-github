import { ContributionLevelName, CONTRIBUTION_LEVELS, isValidContributionLevelName } from './contributions'
...
const run = (str: string, code: Code): string => `${code.open}${str.replace(code.regexp, code.open)}${code.close}`

const clampAndTruncate = (n: number, max = 255, min = 0): number => Math.trunc(Math.max(Math.min(n, max), min))

export const rgb = (str: string, color: number | Rgb): string => {
...
const isValidColorSchemeName = (name?: keyof typeof COLOR_SCHEMES): name is ColorSchemeName =>
    !!name && !!COLOR_SCHEMES[name]
...
export const getColorScheme = (name: keyof typeof COLOR_SCHEMES | 'random' | string = 'github') => {
    if (name != 'random' && !isValidColorSchemeName(name as keyof typeof COLOR_SCHEMES)) {
        throw new Error(`'${name}' is invalid color scheme name! Choose from: ${Object.keys(COLOR_SCHEMES)},random`)
    }

    const hexStrColors = name === 'random' ? randomColorScheme() : COLOR_SCHEMES[name as keyof typeof COLOR_SCHEMES]

    const hexNumColors = hexStrColors.map((color) => hexStrToHexNum(color))

    const getByLevel = (levelName?: ContributionLevelName) =>
        hexNumColors[isValidContributionLevelName(levelName) ? CONTRIBUTION_LEVELS[levelName] : 0]

    return { hexStrColors, hexNumColors, getByLevel }
}

const hexStrToHexNum = (color = defaultColor) => parseInt('0x' + convertToSixChars(color))

const confirmHex = (str: string) => (/^#?([0-9a-f]{3}){1,2}$/i.test(str) ? str : defaultColor)

const convertToSixChars = (str: string) =>
    confirmHex(str).replace(/^#?(.*)$/, (_, hex) => (hex.length == 3 ? hex.replace(/./g, '$&$&') : hex))
